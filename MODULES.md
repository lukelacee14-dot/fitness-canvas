...
- **Universal Save Confirmation** — One shared bottom-sheet component (js/save-confirm.js) triggered by every save/log action across the app, offering "Save" (unchanged, no timeline post) or "Save & Post to Timeline" (saves the entry and drops a matching post into Activity Feed's own storage, rendered with identical styling to a post made from the feed's own composer). Each tool auto-generates a one-line summary of what it just saved (e.g. "Completed a 5.2 Run — 32:10, avg pace 6:10/mi", "Logged Chicken Breast & Rice — 620 kcal", "Logged Bench Press — 3 sets × 5 reps × 185 lbs/kg"), pre-filled into an editable caption field before posting. Wired into every save/log action in: Workout Logger (Strength Training's Add Set, and every sport's generic Add Entry — including GPS-tracked activities, which flow through the same form), Meal Tracker (new food item, database search, and saved-meal logging all funnel through one shared save point), Body Measurements, Sleep Tracker, Soreness/RPE Log, and Progress Photos. Not wired into Meal Tracker's water widget (a continuous +/- counter with no discrete "save" moment) or template-creation actions like Program Builder's "Save to My Programs" or Recipe Builder's "Save to Saved Meals" (those save a reusable plan/template, not a dated logged activity). — Live
