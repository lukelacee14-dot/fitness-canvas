...
## Training
- **Workout Logger** — Multi-sport activity logger with its own internal sport tabs; a data-driven template system so each sport logs the fields that make sense for it. Individual sets/entries save quietly (no per-entry confirmation sheet) so a real session doesn't interrupt itself after every set or run. A module-level session counter (in-memory, spans every sport used in the session) surfaces a "Finish Session" button pinned above the bottom nav once anything's been logged; finishing it resets the counter and, only if a profile exists, opens a one-time summary prompt with an editable caption rolled up from everything logged that session, offering "Post to Timeline" or "Skip". Newly-saved rows get a brief ember flash to acknowledge the save without a dialog. — Live
...
- **Ironhearth Design System** — Superseded the earlier blue/light color pass with the full Ironhearth system from BRAND.md, applied app-wide: Canvas `#14161C` (app background everywhere), Surface `#1E212B` (cards, panels, modals, nav bars, screen-header banners — one flat elevation step, no drop shadows), Ember Accent `#FF6B35` (the only accent — primary actions, active/selected states, highlights), Text Primary `#F2F1ED` / Text Secondary `#9B9CA8`, plus Error `#FF5C5C` for destructive actions and a translucent divider tone for structural list/table separators. Icons default to Text Secondary, switching to Ember only when active/selected. Typography uses Inter (loaded from Google Fonts, system-font fallback) with BRAND's H1/H2/H3 scale applied to real headings (app title, screen banners, modal/section/card titles); dense secondary UI text keeps its existing compact sizing. Every button/input/interactive row has consistent hover (pointer devices), pressed, and disabled states via a brightness-filter approach that introduces no colors outside the palette; `prefers-reduced-motion` disables non-essential animation. The app icon, favicon, and PWA splash screen now use the real Ironhearth brand artwork from /assets/brand/ (icon-1024.png, splash-screen.png) in place of the earlier placeholder icon set. No tool is left on its old ad hoc colors. — Live
- **Custom Tool Icon Set** — Replaced the emoji placeholders used for every top-level tool/module (bottom tab bar and each tool's screen-header banner) with the custom bold/geometric icon set from BRAND.md, sourced from /icons/tools/. Each of the 17 tools (Workout Logger, Program Builder, Exercise Library, Rest Timer, PR Tracker, Meal Tracker, Water Tracker, Recipe Builder, Body Measurements, Progress Photos, Sleep Tracker, Soreness/RPE Log, Progress & Analytics, Streaks, Activity Feed, Leaderboards, Achievements) has a Text Secondary default PNG and an Ember Accent active PNG, swapped by state exactly like the rest of the icon color rules — Streaks is the one exception, always rendering its ember-toned flame regardless of active state per BRAND.md. Account Bar items (Personal Profile, Sport Info Lookup, Search People, Connect Device) still use emoji, since the custom set only covers the 17 top-level tools. — Live
- **Universal Save Confirmation** — One shared bottom-sheet component (js/save-confirm.js) triggered by every save/log action across the app, offering "Save" (unchanged, no timeline post) or "Save & Post to Timeline" (saves the entry and drops a matching post into Activity Feed's own storage, rendered with identical styling to a post made from the feed's own composer). Each tool auto-generates a one-line summary of what it just saved (e.g. "Logged Chicken Breast & Rice — 620 kcal"), pre-filled into an editable caption field before posting. "Save & Post to Timeline" only makes sense once there's a profile to post as, so the sheet checks for one (a non-empty name in module:profile); with no profile it skips the sheet entirely and saves directly, no dialog. Wired into every save/log action in: Meal Tracker (new food item, database search, and saved-meal logging all funnel through one shared save point), Body Measurements, Sleep Tracker, Soreness/RPE Log, and Progress Photos. Not wired into Meal Tracker's water widget (a continuous +/- counter with no discrete "save" moment), template-creation actions like Program Builder's "Save to My Programs" or Recipe Builder's "Save to Saved Meals" (those save a reusable plan/template, not a dated logged activity), or Workout Logger anymore (see its own entry above — it moved to a per-session summary instead of a per-entry one, reusing this same profile check and feed-posting logic rather than the sheet itself). — Live
- **Tab Long-Press Lift Feedback** — Holding a bottom-bar tool icon now gives iOS-style "hold to remove" feedback before the remove confirmation appears: shortly (~100ms) into the hold, the icon scales up and lifts slightly with a soft ember ring, animated with the app's standard 200ms transition; releasing early settles it back down, and once the 550ms threshold is reached and the remove-confirm dialog opens, the lift clears immediately so the dialog has focus instead of a still-lifted icon sitting underneath it. Purely a visual cue — the existing long-press threshold and remove flow are unchanged. — Live
